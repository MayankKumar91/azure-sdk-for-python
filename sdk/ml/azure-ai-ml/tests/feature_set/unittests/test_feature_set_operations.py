...
class TestFeatureSetOperations:
    ...
    def test_get_with_version(self, mock_feature_set_operations: FeatureSetOperations) -> None:
        ...
        with patch.object(ItemPaged, "next"), patch.object(FeatureSet, "_from_rest_object", return_value=feature_set):
    ...
    def test_archive_version(self, mock_feature_set_operations: FeatureSetOperations):
        name = "random_name"
        feature_set_version = Mock(FeaturesetVersion(properties=Mock(FeaturesetVersionProperties(entities=["test"]))))
        version = "1"
        mock_feature_set_operations._operation.get.return_value = feature_set_version
    ...
    def test_archive_container(self, mock_feature_set_operations: FeatureSetOperations):
        name = "random_name"
        feature_set_container = Mock(
            FeaturesetContainer(properties=Mock(FeaturesetContainerProperties(description="test")))
        )
        mock_feature_set_operations._container_operation.get.return_value = feature_set_container
