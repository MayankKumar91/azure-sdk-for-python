...
class FeatureSetOperations(_ScopeDependentOperations):
    def __init__(
        ...
        **kwargs: Dict,
        ...
        self._operation = service_client.feature_set_versions
        self._container_operation = service_client.feature_set_containers
    ...
    def get(self, *, name: str, version: Optional[str] = None, label: Optional[str] = None) -> FeatureSet:
        ...
        try:
            ...
            feature_set_version_resource = self._get(name, version)
            return FeatureSet._from_rest_object(feature_set_version_resource)
        except (ValidationException, SchemaValidationError) as ex:
            log_and_raise_error(ex)

    # @monitor_with_activity(logger, "FeatureSet.BeginCreateOrUpdate", ActivityType.PUBLICAPI)
    def begin_create_or_update(self, feature_set: FeatureSet) -> LROPoller[FeatureSet]:
        """Create or update FeatureSet

        :param feature-set: FeatureSet definition.
        :type feature-set: FeatureSet
        ...
        featureset_spec = validate_and_get_featureset_spec(feature-set)
        feature_set.properties["spec_version"] = "1"
        feature_set.properties["spec_data"] = json.dumps(featureset_spec._to_dict())

        sas_uri = None
        feature_set, _ = _check_and_upload_path(
            artifact=feature_set, asset_operations=self, sas_uri=sas_uri, artifact_type=ErrorTarget.FEATURE_SET
        )

        feature_set_resource = FeatureSet._to_rest_object(feature_set)
...
def validate_and_get_featureset_spec(feature_set: FeatureSet) -> FeaturesetSpec:
    # pylint: disable=no-member
    if not feature_set.specification and not feature_set.specification.path:
        msg = "Missing feature-set spec path. Path is required for feature_set."
        raise ValidationException(
            message=msg,
            no_personal_data_message=msg,
            error_type=ValidationErrorType.MISSING_FIELD,
    ...
    featureset_spec_path = str(feature_set.specification.path)
